...
import org.apache.spark.ml.regression.LinearRegressionBatch
 ...
 */
object LinearRegressionBatchExample {
  ...
  case class Params(
    ...
    numRDDpart: Int = 2,
    batchSize: Int = 2,
    fracTest: Double = 0.0) extends AbstractParams[Params]


  def main(args: Array[String]) {
    val defaultParams = Params()

    val parser = new OptionParser[Params]("LinearRegressionBatchExample") {
      head("LinearRegressionBatchExample: an example Linear Regression Batch with Elastic-Net app.")
      opt[Double]("regParam")
  ...
  def run(params: Params) {
    val spark = SparkSession
      .builder
      .appName(s"LinearRegressionBatchExample with $params")
      .getOrCreate()

    println(s"LinearRegressionBatchExample with parameters:\n$params")
   ...
   if (params.batchSize > 0) {
      var batch = new Array[LinearRegression](params.batchSize )

      var i = 0
      for (i <- 0 until batch.size) {
        batch(i) = new LinearRegression()
      ...
      val lir_batch = new LinearRegressionBatch(batch)
      val lir_return = lir_batch.train(training, params.numRDDpart).collect()
         
      printf("*****%d batch took %d ms\n", lir_return.size, System.currentTimeMillis() - start_time);
      lir_batch.summarize(training,lir_return) 
    }
